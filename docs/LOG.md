...
- **Claude Code — image/visual polish pass (chat-driven, iterative):** Om oss hero finalized on `about-hero-maher-customers` (final crop `Klar_maher ute_man_kvinna 2`, Maher extended further left of frame, out from under the headline) with a light 9% dark overlay restored for text contrast — resolves the open item in the entry below. Bärgning: removed the duplicate "Snabb hjälp när du behöver det" 3-step section entirely (incl. the now-unused `GarageIcon`); the showcase card now follows the hero directly — resolves "two near-identical 3-step flows" in STATUS. Header `.public-header__booking` gradient darkened. Landing `why-section`: new photo `landing-why-reassurance-handshake-light` (full-bleed, no dark overlay, edge-faded to page colour top/bottom, flat white wash), "Läs mer om oss" button removed, and its four reassurance items moved out into a new `.landing-v2__trust-strip` row above the services grid, which now carries its own light-to-dark-teal gradient background with white titles/body text. Om oss: quote card recoloured to a teal gradient (white text); principles section ("Därför väljer kunder oss") gained a new background photo (`about-principles-maher-customer-bg`, edge-faded + flat page-colour wash) and its four items moved from a 2×2 grid beside the heading into one row below it. Manual browser checks at 1440/900/390 only — Playwright not re-run this session.
- **Claude Code — editorial rhythm (branch `polish/editorial-rhythm`, checkpoint tag `checkpoint/pre-editorial-rhythm`, uncommitted pending Magnus's approval of copy and visuals):** narrative card groups turned into text. Two family layouts in `ServiceReparationerPage.css`: `.bilservice__editorial` (heading column + `.bilservice__prose`) and `.bilservice__ledger` (ruled `<dl>` rows). Bilservice "Varför är bilservice viktigt?" is one photo + prose (three photos no longer imported); Felsökning areas as a ledger, "Bra att veta" as prose with a phone button; AC "Varför boka" and "Så håller du koll" as prose; Däckservice "Mer om" drops the checklist that repeated the tyre grid. Guides: `GuideImportance` renders prose on the dark panel, `GuideInfo` a ruled ledger, `GuideTopic` gains `variant="prose"` (Oljebyte viscosity, standards, oil types). Om oss principles: heading column beside four text blocks, no card chrome or hover lift. Kept: price/tier/tyre cards, symptom selectors, checklists of inclusions, process steps, FAQ, Bärgning scenarios. Suite 205 passed; `/` and `/galleri` desktop text baselines were already stale from `2941d489`/`abff82c1`.
- **Claude Code — round 2 after Magnus's review ("onödiga rubriker, dåliga radbrytningar, fula kort"):** the generic 5-step "Så går det till hos oss" removed from the ten guides, Felsökning and Däckservice (kept on Landing and Service & reparationer; `GuideProcess` deleted); "Det här kan vi hjälpa dig med" and "Mer info"/"Viktig information om …" replaced by page-specific headings (`GuideServiceCard` heading now required); family layouts collapse at 900px instead of 1024px; balanced headings; symptom cards restyled as light choice cards; recommendation-bar button switched to the light-surface variant. Suite 205 passed (same two stale baselines).
- **Claude Code — round 3, more text (Codex's editorial brief):** customer-question paragraphs added and unsupported general claims removed on Service & reparationer (serviceprogram, kostnadsförslag/15 %), Felsökning (felkod ≠ orsak, describe when), Däckservice (balancing vs alignment vs damage; "vid vinterväglag"; 3PMSF for studless), AC (service frequency, cooling/smell/leak), all ten guides, Landing and Biltjänster (symptom pointer). Removed: kamrem 60 000–100 000 km/6 år and "kamkedja håller bilens livslängd", koppling/broms mileage figures, "20 procent" braking distance, battery-damages-generator, oil-fuel-economy claim, Oljebyte duplicates. Sources checked: Transportstyrelsen, Konsumentverket, Volvo (search), Michelin, Brembo, VARTA, KYB, Continental, Schaeffler (search), Walker, GKN (search), ZF, Hella; Naturvårdsverket's page returned 404. Suite: only text baselines and disk-full (ENOSPC, 764 MB free) failures; re-run of the flaky specs 23/24, the last failing on ENOSPC.
- **Claude Code — Landing:** the two "om oss" sections merged into the dark one (eyebrow "Om Brynäs Bilservice", the about text, one "Läs mer om oss"); `.landing-v2__about-*` markup and CSS removed; `landing.visual.spec.ts` screenshots the merged section.
- **Claude Code — Om oss hero:** new photo `about/about-hero-maher-customers` (Kontakt and the Om oss CTA card keep `about-hero-bg`); the hero's dark gradient overlay removed at Magnus's request. Open: body text is hard to read over the light wall, worst on phones.
...
- **Claude Code — guard for the shared pieces:** `shared-components.spec.ts` scans `src/` and fails when tip, booking-modal, contact-sending or guide-section markup is written outside its owner file, and when an owner stops matching its own rule; both checked by planting offenders. Full suite 207 passed, 12 skipped.
