...
3. **"Bilservice" family (shared template)** — major service-hub pages, all structurally the same kind of page:
   ...
   - *Pages on this template*: `Bilservice` (`/service-reparationer`), `Felsökning` (`/felsokning`), `Däckservice` (`/dackservice`), `AC-service` (`/ac-service`). AC uses the family stylesheet for its content sections and a full-bleed `.bb-hero` with standard booking and phone actions, without a registration form.
