...
3. **"Bilservice" family (shared template)** — major service-hub pages, all structurally the same kind of page:
   ...
   - *Pages on this template*: `Bilservice` (`/service-reparationer`), `Felsökning` (`/felsokning`), `Däckservice` (`/dackservice`), `AC-service` (`/ac-service`). AC uses the family stylesheet for its content sections and a full-bleed `.bb-hero` with standard booking and phone actions, without a registration form. Felsökning, Däckservice and AC take their hero height from the shared `--bb-hero-matched-height` token, like Landing, Om oss, Bärgning and Kontakt.
   ...
   - *Text layouts (2026-09-25):* `.bilservice__editorial` (heading column `.bilservice__editorial-head` beside `.bilservice__prose`; stacks at 1024px) and `.bilservice__ledger` (ruled `<dl>` term/description rows, two columns, one below 640px). Use them for reasons, advice and parallel facts instead of a card grid; `.bilservice__prose` also works as the text column of `.bilservice__split`. `.bilservice__nowrap` keeps "AC-service" from breaking at its hyphen. Both families collapse to one column at 900px (was 1024px, which made 1024-wide windows show full-width photos), stacked split photos are capped at 360px, and headings use `text-wrap: balance`, body text `text-wrap: pretty`. Symptom selectors are light choice cards with a radio-style marker (`::after`); their inner elements are `<span>`s because a `<button>` may only hold phrasing content. The old `.bilservice__value-*`, `__card-grid-3`, `__card--teal` and `__stat-*` rules were deleted with their last users.
   - *Spacing (reduced 2026-09-24 to match Landing rhythm):* `.bilservice__section` `clamp(3rem, 4.5vw, 4rem)`, `--tight` `clamp(2rem, 3.5vw, 2.5rem)`, `--flow-bottom` `clamp(2rem, 3vw, 2.5rem)`, `--pad` `clamp(2.5rem, 4vw, 3rem)`, `--pad-lg` `clamp(2.5rem, 4vw, 3.5rem)`, and `.bilservice__container--flow` top `clamp(1rem, 2vw, 1.5rem)` / bottom `clamp(2rem, 4vw, 3rem)`. The `--wide` 78ch / `--wide-sm` 75ch and `--intro` 0.5rem / `--intro-tight` 0.4rem near-duplicates remain intentional.
...
| `/ac-service` | Bilservice family | Complete — family sections plus a full-bleed hero and standard booking/phone actions; mounts `PublicHeader` (overlay) + `PublicFooter` | `ServiceReparationerPage.css` (`.bilservice__ac-*`, `.bilservice__*`) |
