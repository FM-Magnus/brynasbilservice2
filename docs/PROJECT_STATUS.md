...
Only after an image is selected should it be exported and added to `client/src/assets/images/`. The 2026-09-15 asset pass uses content-based folders (`brand/`, `home/`, `gallery/`, `people/`, `services/` and `vehicles/`), WebP as primary with JPG fallback when the consuming component supports both, and `-thumb` pairs for the Peugeot image picker. See `_incoming-assets/README.md` and `_incoming-assets/ASSET_INVENTORY.md` for the authoritative local source-bank workflow and current mapping. Match the actual placement and crop when each asset is integrated; this dashboard sets no universal pixel dimensions. Keep business claims and prices subject to Magnus's approval.
