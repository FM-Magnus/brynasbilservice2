...
The desktop Biltjänster dropdown currently orders **Våra tjänster → `/biltjanster`** before **Bilservice → `/service-reparationer#bilservice`**. Mobile Biltjänster links to `/biltjanster`. The old `/tjanster` route remains reachable from page CTAs; its future role has not been decided.
