...
**Last updated:** 2026-09-25 (Claude Code — editorial rhythm plus an image/visual polish pass, on branch `polish/editorial-rhythm`; check `git status -sb` for push/merge state).
...
- **Imagery:** every guide and service page has its slots filled with WebP+JPG pairs; per-page detail in [`IMAGES.md`](IMAGES.md). Optional hero swaps (Koppling, Stötdämpare) are listed there. Every production image is imported somewhere, except three kept on purpose (the black logo variant, the Maher portrait, and now `landing-why-reassurance-handshake-v2.jpg` — superseded by `landing-why-reassurance-handshake-light`) and, since the editorial-rhythm change, `services/general/service-{longevity-oil-filter,performance-diagnostics,resale-maintained-car}` — reuse or delete them. `images/workshop/` holds fixed exports for other pages; `assets/galleri/` drives `/galleri` only.
...
0. **Editorial rhythm, committed (Magnus approved 2026-09-25):** branch `polish/editorial-rhythm` (not pushed or merged into `redesign/blue-teal-v1`; checkpoint tag `checkpoint/pre-editorial-rhythm`) replaces narrative card groups with prose/ledger layouts on Bilservice, Felsökning, AC, Däckservice, the ten guides and Om oss; round 2 removed the generic process block and templated headings, moved the one-column breakpoint to 900px and restyled the symptom cards. Round 3 added customer-question text and removed unsupported claims (list of company facts for Maher in the 2026-09-25 LOG entry and the chat report). Bärgning's duplicate 3-step flow is resolved (see LOG). **The Mac's disk is full (≈0.8 GB free), which makes Playwright fail with ENOSPC.**
   Om oss hero: finalized on `Klar_maher ute_man_kvinna 2` (extended left of frame) with a light 9% dark overlay restored for text contrast — no longer an open decision. Om oss principles section and quote card, and the Landing "why" section, were reworked the same session (see LOG 2026-09-25) — not yet Playwright-verified beyond manual browser checks at 1440/768 or 900/390.
