...
## Start here (updated 2026-09-21, after hero step 3)

- **Repo / branch:** `/Users/magnusolsson/repos/brynasbilservice2`, branch `redesign/blue-teal-v1` (remote `origin` = `FM-Magnus/brynasbilservice2`). **Everything from 2026-09-20 was pushed to `origin/redesign/blue-teal-v1` at Magnus's explicit instruction** (about 28 commits). Hero step 3 (2026-09-21) was committed after that and is **not pushed** at the time of writing. Check `git status -sb` before assuming any of this is still true. Never push without Magnus's explicit go-ahead.
...
- **Read next:** [`AGENTS.md`](../AGENTS.md) (contract), [`CSS_OWNERSHIP.md`](CSS_OWNERSHIP.md) (route → CSS owner map, write rules), [`DESIGN_SYSTEM.md`](DESIGN_SYSTEM.md) (tokens, patterns, §2b icons, §2c hero geometry), [`SESSION_LOG_CURRENT.md`](SESSION_LOG_CURRENT.md) (newest entries first), [`BACKEND.md`](BACKEND.md).

## What the 2026-09-20 and 2026-09-21 sessions finished (all committed)

- **Booking form:** lifecycle (`hooks/useFormSubmission.ts`: in-flight lock, timeout, inline success/error, no `alert()`), local `yyyy-MM-dd` date (a UTC timestamp was storing Swedish bookings a day early), typed payload, `tests/browser/booking-form.spec.ts`. Server side still to do: `BACKEND.md` §2.3.
...
- **Hero (audit point 4), steps 1–3 of 3:** header-derived clearance, smaller H1 (max 48px), tighter gaps (2026-09-20); then the ten guides adopted the shared `.bb-trust-row` in place of their own trust-row CSS (2026-09-21). Status per page in `DESIGN_SYSTEM.md` §2c.

## Where to pick up (in this order unless Magnus says otherwise)

1. **Hero, remaining outliers** (step 3 is done; per-page status and the reasons are in `DESIGN_SYSTEM.md` §2c). The agreed Standard target is about 85% of a 1280×720 screen; Koppling, Bromssystem and Avgassystem sit at 92–94%, Oljebyte and Kamrem at 105% (image column), AC-service at 125%, and Om oss and Bilar till salu are set by their image or side column. Each needs a design decision from Magnus first. Show before/after images before committing, and re-run `docs/audit-harness/hero/`. Mobile hero tokens are unchanged. The phone-button label has six copy variants (a copy decision for Magnus).
...
6. **Small leftovers:** round caps on the 22 shared icons that use butt caps; shared equivalents for Landing's four local icons; the six-way phone-label copy variants.
   **Hygiene backlog from the 2026-09-20 health check** (no behaviour change; verify as usual): unused dependencies `classnames` and the stale `@types/axios` stub; two debug `console.log`s at `components/admin/BookingManagement.tsx:155-156` that print the sorted bookings (customer data); `SUBMISSION_TIMEOUT_MS` in `hooks/useFormSubmission.ts` is exported but unused, and 18 other exports are never used outside their file (mostly props/type exports, plus `defaultGoogleReviews`, `defaultWorkshopSlides`, `publicServiceNavigation`); about six dead selectors (`service-guide__btn--outline`, `--primary`, `title-accent`, `bilservice__image-slot--radius-md`; `bilservice__level-card--01..03` are built dynamically and are fine); boilerplate repeated per page (`isModalOpen` state in 18 pages, `scrollTo(0,0)` in 11, header mount in 13), removable with a small hook but it touches 18 pages; `SESSION_LOG_CURRENT.md` is about 1,100 lines and should have older entries rotated into the archive; five superseded handovers (`CODEX_HANDOVER`, `ANTIGRAVITY_HANDOFF`, `CLAUDE_CODE_HANDOVER`, `PROJECT_STATUS`, `BILTJANSTER_LAYOUT_PLAN`) could move to `docs/archive/`. Needs Magnus's decision first: the 2.1 MB unused `client/src/assets/images/archive/home-heros/` image, `_magnus/`, `instructions.md`, the three root `*.disabled` files and the orphan root `package-lock.json`.
   **Health check result (2026-09-20):** 0 import cycles, no layering violations, no unimported files, 0 `any`/`@ts-ignore`, guide pages share only 8% of their lines (data-driven, not copy-pasted), about 11 possibly-dead selectors out of about 800. The one structural risk is the hero rules living in four owner files (shared, guides, Om oss, Bärgning), which can drift; the tokens and `hero.spec.ts` reduce that risk.
7. **Push:** ask Magnus.
...
- Snapshots in `tests/browser/baseline-snapshots/` are computed-style fingerprints; a hero H1 change legitimately updates them, and so does a markup change that alters block structure (the guides' `.text.txt` files lost blank lines when their trust items changed from `h3`/`p` to `b`/`small`; prove such a diff is whitespace-only with `git diff --ignore-blank-lines`). Read the diff first.
- This checkout sits in an iCloud-synced folder and files have gone missing from it once (see the 2026-09-21 log entry and the harness README). Run `git status` before a test run, and never commit with `git add -A`.
...
2. **Backend wiring and handoff:** Johnny executes `docs/BACKEND.md` (session auth, `comment_customer`, vehicle CRUD and uploads). The frontend halves are described above.

## Other open follow-ups

1. **P0 security:** admin auth is client-side only; credentials ship in the public bundle and the server accepts a fixed token. `admin123` is in pushed git history and must be rotated. Plan: `BACKEND.md` §2.1. Johnny owns `server/`.
