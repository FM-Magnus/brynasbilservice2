...
4. **comment_customer not saved (server side)** — the client sends it correctly (contract and the two-line fix: `BACKEND_HANDOFF.md` §2.3), but `server/index.js` `insertBooking()` omits it from the INSERT. Vehicle inquiries ("Gäller förfrågan om …") depend on it.
