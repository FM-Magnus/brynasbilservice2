...
- **Om oss section** (`About.tsx`) — Two-column warm-white layout based on approved design: features the reusable `GalleryTeaserCard` component (Ken Burns workshop slideshow using optimized WebP thumbnails in `client/src/assets/images/gallery/`, glass "Grundat 2021" badge, and organic corner cutout "TILL GALLERIET" linking to `/galleri`), verified Swedish copy, teal CTA button linking to `/om-oss`, and two white reassurance cards (`Tydlig kommunikation` & `Omsorg om din bil`). Respects `prefers-reduced-motion`.
- **Reusable Workshop Gallery Teaser Card** (`GalleryTeaserCard.tsx` at `client/src/components/ui/GalleryTeaserCard.tsx`) — Standalone, reusable interactive card (`<a>` linking to `/galleri`). Features rotating Ken Burns workshop slideshow using sharp, lightweight thumbnails (< 70 kB WebP with JPG fallback) in `client/src/assets/images/gallery/` (`workshop-lifts-thumb`, `workshop-tire-machine-thumb`, `workshop-workbench-thumb`, `workshop-car-lift-thumb`), frosted "Grundat 2021" badge, and an organic bottom-right corner cutout (`background: var(--redesign-page); border-top-left-radius: 20px;`) with active slide indicators, uppercase "TILL GALLERIET" text, and circular arrow badge with hover animation. Zero dependencies on temporary `_incoming-assets` folder.
...
- **Dedicated Galleri page** (`GalleryPage.tsx` at `/galleri`) — Dedicated workshop gallery subpage created from the Om oss structure. Features dark hero (`Bilder & Verkstadsmiljö`, `Bilder från Brynäs Bilservice`), full 4-card workshop photo grid with tags/descriptions, gallery expansion TODO comment, closing dark CTA card, booking modal, and verified 0px overflow across 1440px, 768px, and 390px.
