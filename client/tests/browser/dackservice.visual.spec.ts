...
test('däckservice & hjulskifte page renders without horizontal overflow across breakpoints', async ({ page }, testInfo) => {
  ...
  const tireGridBox = await page.locator('.bilservice__tire-grid').boundingBox()
  const datesBox = await page.locator('.bilservice__dates-banner').boundingBox()
  expect(tireGridBox).not.toBeNull()
  expect(datesBox).not.toBeNull()
  expect(datesBox!.y).toBeGreaterThan(tireGridBox!.y + tireGridBox!.height)
