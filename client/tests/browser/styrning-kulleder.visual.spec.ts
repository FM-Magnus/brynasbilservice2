...
test('styrning och kulleder page renders without horizontal overflow across breakpoints', async ({ page }, testInfo) => {
  await page.goto('/styrning-kulleder')
  ...
  await expect(h1).toContainText('Styrning & kulleder')
  ...
  const callBtn = page.locator('.service-guide__actions a[href="tel:0705533395"]').first()
  ...
  await page.screenshot({
    path: testInfo.outputPath(`styrning-kulleder-${testInfo.project.name}.png`),
    fullPage: true,
  })
})
