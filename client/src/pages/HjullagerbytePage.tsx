    ...
    <>
      ...
      <main className="services-page wheel-bearing-page">
        ...
        <section className="services-page__guide wheel-bearing-page__intro" aria-labelledby="wheel-bearing-intro-title"><div className="container">
          ...
          <div className="wheel-bearing-page__parts-rows">{parts.map(([title, text], index) => <article className="wheel-bearing-page__part-row" key={title}><span className="wheel-bearing-page__part-letter" aria-hidden="true">{String.fromCharCode(65 + index)}</span><div><h3>{title}</h3><p>{text}</p></div></article>)}</div>
        ...
        <section className="wheel-bearing-page__guidance" aria-labelledby="wheel-bearing-guidance-title"><div className="container"><div className="services-page__guide-intro"><h2 id="wheel-bearing-guidance-title">Viktig information om hjullager</h2><p>Här är praktiska riktlinjer och fakta kring hjullagrets funktion och underhåll. Vi undersöker alltid bilens faktiska skick innan vi föreslår åtgärder.</p></div><div className="wheel-bearing-page__guidance-grid">{guidance.map(([title, text], index) => <article className={`wheel-bearing-page__guidance-card${index % 3 === 0 ? ' wheel-bearing-page__guidance-card--accent' : ''}`} key={title}><h3>{title}</h3><p>{text}</p></article>)}</div><aside className="wheel-bearing-page__safety-note"><strong>Säkerhetsnotis:</strong> Ett slitet hjullager är inte något att skjuta upp i onödan. Även om missljudet kan pågå en tid finns det en överhängande risk för överhettning eller att lagret skär, vilket i värsta fall kan leda till att hjulet låser sig i hög hastighet.</aside></div></section>
